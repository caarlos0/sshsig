...
func createSignBlob(message []byte, namespace string) ([]byte, error) {
	...
	return append([]byte(sigMagicPreamble), ssh.Marshal(blob{
		Namespace:     namespace,
...
func Sign(s ssh.Signer, rand io.Reader, message []byte, namespace string) ([]byte, error) {
	signer, ok := s.(ssh.AlgorithmSigner)
	if !ok {
		return nil, fmt.Errorf("invalid signer")
	}

	algorithm := ssh.KeyAlgoRSASHA512
	multiSigner, ok := s.(ssh.MultiAlgorithmSigner)
	if ok {
...
func Verify(pub ssh.PublicKey, message, signature []byte, namespace string) error {
	var sig signedData
